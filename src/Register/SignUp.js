...
class SignUp extends Component {
  ...
  handleSubmit(event) {
    ...
    if (this.validate()) {
      ...
      alert('Sign-up Form is submitted');
    }
  }
  ...
  validate() {
    ...
    if (typeof input['id'] !== 'undefined') {
      ...
      if (!pattern.test(input['id'])) {
        isValid = false;
        errors['id'] =
          'Please enter valid user ID. User Id need at least one letter, one number with minimum 8 characters.';
      }
    }
    ...
    if (typeof input['password'] !== 'undefined') {
      let pattern = new RegExp(/^(?=.*[A-Za-z])(?=.*\d)[A-Za-z\d]{12,}$/);
      if (!pattern.test(input['id'])) {
        isValid = false;
        errors['password'] =
          'Please enter valid user ID. User Id need at least one letter, one number with minimum 12 characters.';
      }
    }
